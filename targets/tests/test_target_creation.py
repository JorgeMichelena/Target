...
class CreateTargetTest(APITestCase):
    def setUp(self):
        ...
        self.longitude = randint(-90, 90) + truncate(random(), 5)
    ...
    def test_create_target_when_logged_in_with_incorrect_values(self):
        ...
        response_empty_title = self.client.post('/api/v1/targets/',
                                                data_empty_title)
        response_negative_radius = self.client.post('/api/v1/targets/',
                                                    data_negative_radius)
        response_wrong_location = self.client.post('/api/v1/targets/',
                                                   data_wrong_location)
        response_nonexisting_topic = self.client.post('/api/v1/targets/',
                                                      data_nonexisting_topic)
        self.assertEqual(response_empty_title.status_code,
                         status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response_negative_radius.status_code,
                         status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response_wrong_location.status_code,
                         status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response_nonexisting_topic.status_code,
                         status.HTTP_400_BAD_REQUEST)
